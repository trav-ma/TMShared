...
public extension Date {
    static func daysInMonth(_ monthNumber: Int? = nil, _ year: Int? = nil) -> Int {
        var dateComponents = DateComponents()
        dateComponents.year = year ?? Calendar.current.component(.year,  from: Date.now)
        dateComponents.month = monthNumber ?? Calendar.current.component(.month,  from: Date.now)
        if
...
public extension Date {
    ...
    func dbDate(timeZone: TimeZone? = nil) -> String {
        let dateFormatter = DateFormatter()
    ...
    static func fromMilliseconds(_ milliseconds: Int) -> Date {
        return Date(timeIntervalSince1970: TimeInterval(milliseconds) / 1000)
    }
    
    static var gmt: Date {
        return Date(timeZone: TimeZone(secondsFromGMT: 0))!
    }
